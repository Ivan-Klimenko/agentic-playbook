...
47 common mistakes when building LLM-powered agents, with fixes. Cross-referenced to the relevant pattern documents.
...
| 4 | Using only LLM-routing or only hardcoded routing | Pure LLM-routing is slow for predictable inputs; pure hardcoded routing is rigid | Hybrid: pre-processing shortcuts for known patterns, LLM for the rest ([AGENT_PATTERNS §2.1](./AGENT_PATTERNS.md#21-orchestrator-types)) |
...
| 6 | State machine for agent mode transitions | Over-engineered, hard to debug, special state management | Synthetic messages with agent field ([AGENT_PATTERNS §2.7](./AGENT_PATTERNS.md#27-mode-switching-via-synthetic-messages)) |
...
| 8 | Over-engineering planning (formal plan-and-execute everywhere) | Doubles latency for simple tasks; rigid step ordering limits LLM flexibility | No-plan for short interactions; prompt-driven plan for complex ones ([AGENT_PATTERNS §2.10](./AGENT_PATTERNS.md#210-no-plan-architecture-planning-via-system-prompt)) |
...
| 10 | Storing cheaply-derivable or formatted data in graph state | State bloat, stale data, can't format differently per node. (Caching *expensive* derivations is fine.) | Store raw data, derive on read ([LANGGRAPH_PATTERNS §2](./LANGGRAPH_PATTERNS.md#2-state-design-principles)) |
| 11 | One fat node mixing different external services | Single failure kills entire step, can't retry selectively | Split by service/failure mode ([LANGGRAPH_PATTERNS §3](./LANGGRAPH_PATTERNS.md#3-node-granularity)) |
...
| 17 | Sub-agents inherit parent context | Context clash, confusion, poisoning from irrelevant history | Replace messages with task-only context ([AGENT_PATTERNS §3.3](./AGENT_PATTERNS.md#33-context-isolation-via-sub-agents)) |
| 18 | No structured reflection checkpoints | Agent makes impulsive decisions on complex multi-step tasks | think_tool forces articulated reasoning ([AGENT_PATTERNS §3.5](./AGENT_PATTERNS.md#35-think-tool-no-op-forced-reflection)) |
| 19 | Editing/reordering earlier messages between turns | Invalidates prompt cache, re-processes entire context at full cost | Append-only context ([AGENT_PATTERNS §3.6](./AGENT_PATTERNS.md#36-prompt-caching-aware-context-design)) |
| 20 | Using expensive model for context summarization | Wastes frontier-model capacity on a compression task | Summarize with cheap model, keep expensive model for reasoning ([AGENT_PATTERNS §3.6](./AGENT_PATTERNS.md#36-prompt-caching-aware-context-design)) |
| 21 | Removing tools from schema as first reaction to limits | Invalidates entire prompt cache (tools sit at top of cache hierarchy) | Return error stub first (cache-safe); only remove tool if LLM ignores the error ([AGENT_PATTERNS §3.6](./AGENT_PATTERNS.md#36-prompt-caching-aware-context-design)) |
| 22 | Unbounded tool output in context | Large outputs flood context, displace reasoning | Auto-truncate at threshold + offload full output to temp file ([AGENT_PATTERNS §3.7](./AGENT_PATTERNS.md#37-tool-output-truncation-with-file-offloading)) |
...
| 45 | Naive compaction drops identifiers | Agent loses UUIDs, file paths, URLs after summarization — references phantom IDs | Identifier preservation policy with explicit summarization instructions ([AGENT_PATTERNS §3.11](./AGENT_PATTERNS.md#311-identifier-preservation-in-compaction)) |
| 46 | Head-only tool output truncation | Error messages, stack traces, and result summaries at the end get cut | Smart head+tail strategy: detect important tails, preserve both ends ([AGENT_PATTERNS §3.7](./AGENT_PATTERNS.md#37-tool-output-truncation-with-file-offloading)) |
...
| 30 | Exact-match-only for tool inputs | LLM whitespace/indentation hallucinations cause ~20% failure rate | Fuzzy fallback chain ([AGENT_PATTERNS §2.12](./AGENT_PATTERNS.md#212-fuzzy-tool-input-resilience)) |
...
| 32 | Adding a tool recommender/router as first optimization | Over-engineering; usually tool descriptions or ordering are the real problem | Improve descriptions, reduce set via policy, fix ordering first ([AGENT_PATTERNS §2.11](./AGENT_PATTERNS.md#211-pure-llm-tool-selection-no-planner-no-router)) |
| 43 | Clearing tool error state on any tool success | Agent thinks mutating operation succeeded when only a read succeeded | Action fingerprinting: clear error only when same action succeeds ([AGENT_PATTERNS §2.14](./AGENT_PATTERNS.md#214-tool-mutation-tracking-action-fingerprinting)) |
| 44 | Polling for sub-agent completion results | Wastes tool calls, pollutes parent context, adds latency | Push-based announce: deliver results as user messages ([AGENT_PATTERNS §2.15](./AGENT_PATTERNS.md#215-push-based-sub-agent-results-no-polling)) |
...
| 34 | Same model for all cognitive loads | Expensive model wasted on summarization/formatting | Two-tier: cheap model for extraction, expensive for reasoning ([AGENT_PATTERNS §2.9](./AGENT_PATTERNS.md#29-two-tier-model-strategy)) |
...
| # | Anti-pattern | Why it's bad | Fix |
|---|---|---|---|
| 47 | Treating all LLM provider errors the same | Hammers rate-limited provider instead of backing off; applies cooldown to timeouts | Error-typed failover: classify by reason, select per-type recovery ([AGENT_PATTERNS §2.4](./AGENT_PATTERNS.md#24-error-boundaries--graceful-degradation)) |
